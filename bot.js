...
client.on('interactionCreate', async interaction => {
  ...
  if (commandName === 'verify') {
    ...
    const embed = new EmbedBuilder().setColor(0x57F287).setTitle('Verification Successful').setDescription(`${user} has been verified with code \`${code}\``).setTimestamp();
  ...
  if (commandName === 'linkroblox') {
    ...
    if (role) await member.roles.add(role).catch(() => {});
    const embed = new EmbedBuilder().setColor(0x5865F2).setTitle('Roblox Account Linked').setDescription(`${user} linked Roblox account: **${username}**`).setTimestamp();
...
client.on('messageCreate', async message => {
  ...
  if (currentWarnings === 0) {
    ...
    const embed = new EmbedBuilder().setColor(0xED4245).setTitle('User Banned').setDescription(`${message.author.tag} was banned for repeated violations.`).setTimestamp();
...
console.log('Starting Discord login now...');

client.login(TOKEN)
  .then(() => console.log('client.login resolved'))
  .catch((err) => console.error('LOGIN FAILED:', err && err.message ? err.message : err));

const rest = new REST({ version: '10' }).setToken(TOKEN);
const timeout = new Promise((_, reject) => setTimeout(() => reject(new Error('REST timeout after 8s')), 8000));

Promise.race([rest.get(Routes.user('@me')), timeout])
  .then((me) => console.log(`REST token OK. Bot user: ${me.username}`))
  .catch((err) => console.error('REST token check failed:', err && err.message ? err.message : err));
